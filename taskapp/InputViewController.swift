...
class InputViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        categoryTextField.text = task.category // カテゴリタスク追加 2/20
    ...
    override func viewWillDisappear(_ animated: Bool) {
        try! realm.write {
            ...
            self.task.category = self.categoryTextField.text! // カテゴリタスク追加 2/20
    ...
    func setNotification(task: Task) {
        ...
        // タイトルとカテゴリと内容を設定(中身がない場合メッセージ無しで音だけの通知になるので「(xxなし)」を表示する)
        if task.title == "" {
